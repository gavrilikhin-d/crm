...
function DialogContent({
  ...
  const visualViewportStyle = useDialogVisualViewportStyle()
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "fixed top-1/2 left-1/2 z-50 grid w-full max-w-[calc(100%-2rem)] -translate-x-1/2 -translate-y-1/2 gap-4 rounded-xl bg-popover p-4 text-sm text-popover-foreground ring-1 ring-foreground/10 duration-100 outline-none max-sm:top-[calc(var(--dialog-visual-viewport-offset-top)+1rem)] max-sm:max-h-[calc(var(--dialog-visual-viewport-height)-2rem)] max-sm:translate-y-0 max-sm:overflow-y-auto max-sm:overscroll-contain sm:max-w-sm data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95",
          className
        )}
